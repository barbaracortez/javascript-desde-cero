...
function datosObj(obj) {
  ...
  // obtener los valores nombre y apellido y dar un mensaje de bienvenida
...
function permisosDeEntrada(personas) {
  ...
  // tu código:
  const personasPermitidas = personas.filter(
    (persona) => persona.entrada && persona.edad >= 18
  );
  return personasPermitidas.map((persona) => persona.nombre);
}
...
function mostrarInformacionLibro(libro) {
  return `El libro ${libro.titulo}, escrito por ${libro.autor}, fue publicado en ${libro.añoPublicacion}.`;
}
